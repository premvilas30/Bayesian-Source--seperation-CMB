/*modified for the CMB problem from H Rue's modification of the algorithm Tue 23 Oct 2012 12:48:59 IST */
...
static int minimize(gsl_function_fdf * fn, double rho, double sigma, double tau1, double tau2, double tau3, int order, double alpha1, double *alpha_new)
{
	...
	while (i++ < bracket_iters) {
		...
		if (falpha > f0 + alpha * rho * fp0 || falpha >= falpha_prev || FALSE) {
			a = alpha_prev;
			fa = falpha_prev;
			fpa = fpalpha_prev;
			b = alpha;
		...
		if (fabs(fpalpha) <= -sigma * fp0 || FALSE) {
			*alpha_new = alpha;
			return GSL_SUCCESS;
		}
