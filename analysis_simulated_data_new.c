...
int main()
{
	...
	char **template_file = simulated_data_template_file_names();
	...
	spec[0] = -2.7;//SYNC_LOWER + (SYNC_UPPER-SYNC_LOWER)/2.  ;
	spec[1] = 1.7;//DUST_LOWER + (DUST_UPPER-DUST_LOWER)/2.  ;
	
	
	//optimizer
	int optrun = 3, max_iter = 10000, or, window, thread_num;
	...
	//spectral params 
	for( k=0; k<2; k++ ) parinfer[k] = 1;
	
	//precisions for other sources
	for( k=2; k<6; k++ ) parinfer[k] = 1 - templated[k-2];
	...
	//monopoles for each frequncy
	if( monopole )
	{
		//parinfer[k] = 1;
		for( k=9; k<14; k++ ) parinfer[k] = 0;
	}
	...
	model[0] = 1; model[1] = 0; model[2] = 1; model[3] = 0;
	//NNPIXEL;
	...
	int L = n_freq_band *n_sub*n_sub , LL = n_src*n_sub*n_sub;
	double **bigG = calloc( L, sizeof(double*)  );
	for( k=0; k<L; k++ ) bigG[k] = calloc( LL, sizeof(double) );
	...
	for( base = 0 ; base < Npatch; base++ )
	{
		...
		if( individual )
		{
			...
			for( window = 1; window < s->b->n_block + 1 ; window++ )
			{
				...
				//find the loglikelihood of the first entry
				double loglikeinit = model_log_likelihood( pp->mu, pp, hh, FALSE, s->b->mask[window] );
				
				/*printf("\n ======================== \n");
				printf("\n ====== pin template %d  =======\n", pp->nin_template);
				printf("\n ===== window %d =======\n", window ); 
				printf("\n The initial log-likelihood is %lf ", loglikeinit );
				printf("\n This is evaluated for parameters \n\t kappa_s = %lf \n\t kappa_d = %lf \n \
				==========================\n", update_sync_link( hh->theta[0]), update_gdust_link( hh->theta[1] ) );
				
				printf("\n ===== the loadings matrix F ======== ");
				for( k=0; k<5; k++ )
				{
					printf("\n");
					for( l=0; l<4; l++ ) printf(" %lf, ", pp->F[k][l] ) ;
				}
				printf("\n =============== \n");
				
				printf("\n ===== entries of mu template ====== ") ;
				for( k=0; k<5; k++ )
				{
					printf(" %lf, ", ((double*)pp->mu_template->x)[k] ) ;
				}*/
				
				// fill in the entries of bigG
				/*int *pG = (int*) s->p[window]->G->p;
				int *iG = (int*) s->p[window]->G->i;
				double *xG = (double*) s->p[window]->G->x;*/
				
				//printf("\n======== entries of G ======== \n");
				
				//for( k=0; k<200; k++ ) printf("\n %lf", xG[k] );
				
				
				/*int i,j;
				
				printf("\n The nnz of  G is %d", (int)s->p[window]->G->nzmax);
				
				for( j=0; j<pp->G->ncol; j++ )
				{
					for( i=pG[j]; i<pG[j+1]; i++ )
					{
						bigG[ iG[i] ][j] = xG[ i ] ; 
					}
				}*/
				
				/*FILE *fo = fopen("../SIM_small_data/C_mat_kron.txt", "w");
				
				// print G out as a triplet format
				...
				/*for( i=0; i<L; i++ )
				{
					for( j=0; j<LL-1; j++ ) fprintf( fo, "%.20f,", bigG[i][j] );
					fprintf( fo, "%lf\n", bigG[i][LL-1] );
				}*/
				...
				opt[ thread_num ] = optimizer_zzz( s, max_iter, thread_num, 1 ) ; 
				...
				FILE * outf = fopen( "../SIM_small_result/THETA_out.txt",  "a") ; 
				fprintf( outf , "%lf\t" , update_sync_link( s->h[ window ]->theta[0] ) );
				fprintf( outf , "%lf\t" , update_gdust_link( s->h[ window ]->theta[1] ) );
				for( k=2; k<s->h[ window ]->npar; k++ ) fprintf( outf , "%lf\t" , 1./sqrt( update_precision_link(s->h[ window ]->theta[k]) )  ); 
				fprintf( outf , "\n" ); 
				fclose( outf );	
