...
int main(int argc,char **argv)
{
	...
	file[0] = argv[1];
	file[1] = argv[2];//"../WMAP_data/wmap_band_imap_r9_9yr_Ka_v5.fits";
	file[2] = argv[3];//"../WMAP_data/wmap_band_imap_r9_9yr_Q_v5.fits";
	file[3] = argv[4];//"../WMAP_data/wmap_band_imap_r9_9yr_V_v5.fits";
	file[4] = argv[5];//"../WMAP_data/wmap_band_imap_r9_9yr_W_v5.fits";
	//trial
	...
	char **rfile = wmap_get_9yr_result_file_names();
	char *mask_file = wmap_get_9yr_temperature_analysis_mask_name( );
	...
	spec[0] = -2.5;//SYNC_LOWER + (SYNC_UPPER-SYNC_LOWER)/2.  ;
	...
	//reference frequency in units of GHz
	...
	for( base = 0 ; base < Npatch; base++ )
	{
		...
		if( individual ) //if individual is TRUE, process windows indpenedently
		{
		
			// experimental computing of log marginal likelihood
			log_marg_1 = 0;
			...
			for( window = 1; window < s->b->n_block + 1 ; window++ )
			{
				...
				//compute expected values etc of field
